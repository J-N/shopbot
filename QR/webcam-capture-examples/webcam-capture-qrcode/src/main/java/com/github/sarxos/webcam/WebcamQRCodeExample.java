	...
	public void run() {
		...
		do {
			...
			if (result != null) {
				/*
				try {
				    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("/dev/QRComms", true)));
				...
				*/
				try {
					...
					new FileOutputStream("/dev/QRComms")
					);
					System.out.println("Writing to /dev/QRComms");
